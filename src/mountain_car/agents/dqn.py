...
class QNetwork(nn.Module):
    ...
    def __init__(self, state_dim: int, action_dim: int, hidden: int = 128) -> None:
        ...
        self.net = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(hidden, action_dim),  # sin activación: son valores Q
        )
...
class DQNAgent:
    ...
    def __init__(
        ...
        *,
        ...
        self.hidden = hidden
        # Probabilidad de repetir la última acción exploratoria (exploración
        # temporalmente correlacionada, ver Ejercicio 3).
    ...
    def select_action(self, state: np.ndarray, *, deterministic: bool = False) -> int:
        ...
        if not deterministic and random.random() < self.epsilon:
            # Exploración "pegajosa": con prob. sticky_prob se repite la acción
            # exploratoria anterior, produciendo rachas sostenidas (balanceo del
            # carro) en lugar de un paseo aleatorio que no sale del valle.
            if self._last_explore_action is not None and random.random() < self.sticky_prob:
    ...
    def _learn(self) -> float:
        ...
        with torch.no_grad():
            ...
            target_q = rewards_t + self.gamma * next_q * (1.0 - terminateds_t)  # (B, 1)
    ...
    def train(self, total_episodes: int = 500, log_interval: int = 10) -> list[float]:
        ...
        for episode in range(1, total_episodes + 1):
            obs, _ = env.reset()
            self._last_explore_action = None  # estado de exploración por episodio
            total_reward = 0.0
            done = False
