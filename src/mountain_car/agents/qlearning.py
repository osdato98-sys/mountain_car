...
class QLearningAgent:
    ...
    def discretize(self, obs: np.ndarray) -> tuple:
        ...
        """
        # np.digitize con los bordes interiores devuelve un índice en [0, n_bins-1].
        # Valores fuera de rango caen en el primer/último bin (no hay problema:
        # el entorno ya publica los límites físicos de la observación).
    ...
    def _update(
        ...
        """
        # Si el episodio terminó de verdad (bandera alcanzada) no hay bootstrap.
        bootstrap = 0.0 if terminated else self.gamma * float(np.max(self.q_table[next_state]))
